...
class AWAC:
    ...
    def __init__(self, env_fn, actor_critic=core.MLPActorCritic,
        ...
        self.writer = SummaryWriter("runs/" + "MADDPG_shiyan")
    ...
    def populate_replay_buffer(self, env_name):
        ...
        dataset = pickle.load(open( '/home/haojiachen/桌面/AWAC_for_biye/AWAC/left_turn_data_and_scen/dataset', 'rb'))
    ...
    def test_agent(self):
        for j in range(self.num_test_episodes):
            ...
            while not d :
                ...
                o, r, d, collision = self.test_env.step(self.get_action(o, True))
                ep_ret += r
                ep_len += 1
                if collision: collision_time += 1
            self.logger.store(TestEpRet=ep_ret, TestEpLen=ep_len)  # Get unnormalized score
        self.writer.add_scalar("test_reward", ep_ret / 50)
        self.writer.add_scalar("collision_rate", collision_time / 50)
        print('test_reward')
        print(ep_ret / 50)
        print('--------------------------------')
    ...
    def run(self):
        ...
        for t in range(total_steps):
            ...
            # Update handling
            if t > self.update_after and t % self.update_every == 0: #0，50 
                for _ in range(self.update_every):
                    batch = self.replay_buffer.sample_batch(self.batch_size)
                    self.update(data=batch, update_timestep=t)

            # End of epoch handling
            if (t + 1) % self.steps_per_epoch == 0: #100
